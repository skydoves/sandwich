 ...
 */
class ResponseCoroutineCallAdapterFactory private constructor() : CallAdapter.Factory() {
  companion object {
    @JvmStatic
    @JvmName("create")
    operator fun invoke() = ResponseCoroutineCallAdapterFactory()
  }
    ...
    override fun adapt(call: Call<T>): Deferred<ApiResponse<T>> {
      ...
      call.enqueue(object : Callback<T> {
        override fun onFailure(call: Call<T>, throwable: Throwable) {
          deferred.complete(ApiResponse.error(throwable))
        }

        override fun onResponse(call: Call<T>, response: Response<T>) {
          deferred.complete(ApiResponse.of { response })
        }
      })
